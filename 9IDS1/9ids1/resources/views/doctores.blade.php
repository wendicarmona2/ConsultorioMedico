...
<center>
        ...
        <div class="box-body">
            ...
            <thead style=" background-color:#1466C3; color:#fff">
                ...
                <tr>
                    <td class="">{{$doctor['nombre']}}</td>
                    <td class="">{{$doctor['cedula']}}</td>
                    <td class="">{{$doctor['telefono']}}</td>
                    <td class="">{{$doctor['correo']}}</td>
                    <td>{{$doctor->nombre_especialidad}}</td>
                    <td>
                          
                          <a href="{{route('doctor.nuevo',['id' => $doctor['id']])}}" class="btn btn-success btn-sm rounded-8 lote">
                      ...
                      <td>
                                <a>
                                <form action="{{route('doctor.borrar',['id' => $doctor['id']])}}" class="formulario-eliminar" method="POST" >
                                    @csrf
                                    @method('DELETE')
                                    <button  class="btn btn-danger btn-sm rounded-8 lote">
